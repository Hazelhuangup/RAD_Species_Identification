...
./02_src/extract_species_specific_SNPs.py  ./01_raw_data/allsamples121217_SNPs_only.vcf.recode.vcf.gz ./01_raw_data/Sample_to_antirrhinum_names.txt ./01_raw_data/A.bolssierri.SSA.list
...
for i in `ls ./03_output/A.*list`;do
	if [ -s $i ]; then
		./02_src/sliding_window.py $i $i\.sliding_win_freq
	fi
done
